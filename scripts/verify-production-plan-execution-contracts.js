...
const productionScheduleForm = read("..", "frontend", "public", "js", "production-schedule-form.js");
const productionScheduleView = read("..", "frontend", "views", "production", "schedule-form.ejs");
...
verify("Capacity recommendation cannot regenerate work in past dates", capacityRecommendation.includes("executionFloor") && capacityRecommendation.includes("dateOnly(row.scheduleDate) >= today") && capacityRecommendation.includes("replaceableIds"));
...
verify("Capacity UI explains Current Use promotion and history lock", capacityView.includes("Tetapkan sebagai Current Use") && capacityView.includes("Hari yang sudah lewat") && capacityUi.includes("isPastDate"));
...
verify("PR to PO consumes persisted sourcing decisions without reconfirmation", purchaseRequisition.includes("const persistedAllocation") && operationsDetail.includes("finalizedPurchaseOrderLines") && operationsDetail.includes('actionButton("edit-sourcing"'));
