...
function check(code, result, evaluatedAt) {
  ...
  return {
    ...
    actionCode: result.actionCode || null, evaluatedAt, evaluationAttempted: true,
    evaluationOutcome: status === CHECK_STATUS.NA ? "NOT_APPLICABLE" : status === CHECK_STATUS.NOT_CHECKED ? "DATA_MISSING" : "EVALUATED",
...
function runRules(input = {}) {
  ...
  if (!production || supply.applicable === false || supply.externalDependent === false) add("FIRM_SUPPLY_ON_TIME", na(!production ? "Tidak ada production requirement." : "Schedule tidak bergantung pada receipt eksternal."));
    ...
    add("FIRM_SUPPLY_ON_TIME", { status, missingFields: absent.map((key) => `firmSupply.${key}`), requirement: measure(supply.requiredReceiptQty ?? supply.firmReceiptQty, supply.unit || input.uomCode), actual: { ...measure(supply.onTimeFirmReceiptQty, supply.unit || input.uomCode), firmReceiptQty: n(supply.firmReceiptQty), onTimeFirmReceiptQty: n(supply.onTimeFirmReceiptQty), lateReceiptQty: lateQty, unconfirmedReceiptQty: unconfirmed, nextReceiptAt: supply.nextReceiptAt || null, supplierLateDays: n(supply.supplierLateDays) }, gap: measure((n(supply.onTimeFirmReceiptQty) || 0) - (n(supply.requiredReceiptQty ?? supply.firmReceiptQty) || 0), supply.unit || input.uomCode), reason: status === CHECK_STATUS.PASS ? "Dependent receipt firm dan on-time." : status === CHECK_STATUS.WARNING ? "Ada receipt belum confirmed, tetapi masih dalam slack." : status === CHECK_STATUS.FAIL ? "Receipt terlambat/kurang menyebabkan delivery miss." : "Confirmation atau ETA receipt belum lengkap.", evidence: supply.evidence, affectedEntities: supply.affectedPoNumbers, recommendation: status !== CHECK_STATUS.PASS ? "Konfirmasi ETA dan expedite PO/subcontract/transfer terkait." : null, actionCode: status === CHECK_STATUS.FAIL ? "EXPEDITE_SUPPLY" : null });
  }
  ...
  if (!production || lot.applicable === false) add("LOT_BATCH_YIELD_VALID", na(!production ? "MPS Qty 0; lot/batch tidak berlaku." : "Lot/yield constraint tidak diterapkan."));
    ...
    const delta = (n(lot.roundedMpsQty) ?? mpsQty) - mpsQty; const status = lot.valid === false && lot.deliverable === false ? CHECK_STATUS.FAIL : Math.abs(delta) > EPSILON || lot.requiresSplit || lot.requiresYieldAllowance ? CHECK_STATUS.WARNING : CHECK_STATUS.PASS;
    add("LOT_BATCH_YIELD_VALID", { status, requirement: measure(mpsQty, input.uomCode), actual: { ...measure(lot.roundedMpsQty ?? mpsQty, input.uomCode), requestedMpsQty: mpsQty, roundedMpsQty: n(lot.roundedMpsQty) ?? mpsQty, minimumLotQty: n(lot.minimumLotQty), maximumLotQty: n(lot.maximumLotQty), orderMultipleQty: n(lot.orderMultipleQty), plannedBatchCount: n(lot.plannedBatchCount), expectedGoodQty: n(lot.expectedGoodQty), lotRoundingDeltaQty: round(delta) }, gap: measure(delta, input.uomCode), evidence: lot.evidence, reason: status === CHECK_STATUS.FAIL ? "Qty tidak dapat dibentuk menjadi batch valid sebelum due date." : status === CHECK_STATUS.WARNING ? "Qty memerlukan rounding, batch split, atau tambahan untuk yield." : "MPS Qty valid terhadap lot dan yield.", recommendation: status !== CHECK_STATUS.PASS ? "Tinjau rounding, split batch, dan yield sebelum release." : null });
  }
  ...
  else if (!quality.evaluated) add("QUALITY_RELEASE_READY", { ...unknown(quality.missingFields || ["inspectionPlan", "expectedInspectionDuration"], "Pemeriksaan QC dijalankan; data release atau jadwal inspeksi belum lengkap."), evidence: quality.evidence });
  else { const delay = n(quality.qcDelayMinutes) || 0; const status = quality.causesDeliveryMiss ? CHECK_STATUS.FAIL : delay > 0 ? CHECK_STATUS.WARNING : CHECK_STATUS.PASS; add("QUALITY_RELEASE_READY", { status, requirement: measure(null, null, iso(quality.latestAllowedReleaseAt)), actual: { value: quality.stockReleased ? "Usable FG released" : iso(quality.expectedReleaseAt), display: quality.stockReleased ? "Usable FG sudah released" : iso(quality.expectedReleaseAt), unit: quality.stockReleased ? null : "datetime", qcHoldQty: n(quality.qcHoldQty), expectedReleaseAt: iso(quality.expectedReleaseAt), latestAllowedReleaseAt: iso(quality.latestAllowedReleaseAt), qcDelayMinutes: delay, affectedInspectionIds: quality.affectedInspectionIds || [] }, gap: measure(-delay, "minute"), evidence: quality.evidence, reason: quality.stockReleased ? "Demand ditutup usable FG; stock QC hold tidak ikut dipakai." : status === CHECK_STATUS.FAIL ? "QC/release menyebabkan customer arrival terlambat." : status === CHECK_STATUS.WARNING ? "QC release masih on-time dengan buffer kecil." : "Quantity yang dipakai akan released tepat waktu.", affectedEntities: quality.affectedInspectionIds, recommendation: status !== CHECK_STATUS.PASS ? "Prioritaskan inspeksi dan release lot terkait." : null }); }
  ...
  else if (!delivery.evaluated) add("DELIVERY_SLOT_AVAILABLE", { ...unknown(delivery.missingFields || ["dispatchSlot", "transitDuration", "deliveryCalendar"], "Pemeriksaan delivery dijalankan; jadwal kirim atau konfirmasi transport belum lengkap."), evidence: delivery.evidence });
