...
async function buildDemandRows(prisma, filters = {}) {
  ...
  const validSalesTargets = salesOrderTargets.filter((row) => row.soDetail && !row.soDetail.isDeleted && row.soDetail.status !== "Cancelled" && row.soDetail.soHeader && !row.soDetail.soHeader.isDeleted && row.soDetail.soHeader.status !== "Cancelled");
